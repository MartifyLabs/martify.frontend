...
const ButtonPay = ({wallet, create_txn}) => {
  ...
  return (
    <>
      <button className={"button is-primary" + (wallet.loading ? " is-loading" : "")} disabled={wallet.loading || !wallet.connected} onClick={() => begin_buy_process()}>
        <span>Pay</span>
      </button>
      {
        showNotification ? (
...
export default compose(connect(mapStateToProps, mapDispatchToProps))(ButtonPay);
