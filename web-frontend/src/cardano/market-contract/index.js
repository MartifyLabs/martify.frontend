...
export const offer = async (tn, cs, price) => {
  ...
  const walletAddress = instance.BaseAddress.from_address(
    instance.Address.from_bytes(fromHex((await getUsedAddresses())[0]))
  );
...
export const purchase = async (tokenUtxo) => {
  ...
  const requiredSigners = instance.Ed25519KeyHashes.new();
...
export const contractScripts = async () => {
  ...
  const scripts = instance.PlutusScripts.new();

  scripts.add(instance.PlutusScript.new(fromHex(cbor)));
