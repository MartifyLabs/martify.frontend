...
class CardanocliJs {
  ...
  queryProtocolParameters() {
    ...
    const execString = `${this.cliPath} query protocol-parameters \
      --${this.network} \
  ...
  queryUtxo(address) {
    ...
    const execString = `${this.cliPath} query utxo --${this.network} --address ${address} ${this.era}`;
  ...
  queryUtxo_select_utoxs(address, list_utxo) {
    if (this.httpProvider) {
      let response = fetch(`${this.httpProvider}/queryUtxo/${address}`);
      return typeof window !== "undefined"
        ? response.then((res) => res.json())
  ...
  queryUtxo_select_utxos(address, list_utxo) {
    if (this.httpProvider) {
      let response = fetch(`${this.httpProvider}/queryUtxo/${address}`);
      return typeof window !== "undefined"
        ? response.then((res) => res.json())
  ...
  addressBuild(account, options) {
    ...
    const execString = `${this.cliPath} address build \
    ${paymentVkey} \
    ${stakeVkey} \
    ${paymentScript} \
    ${stakeScript} \
  ...
  transactionBuildRaw(options) {
    ...
    // console.log("!! txOutString", txOutString)
    const mintString = options.mint ? mintToString(this.dir, options.mint) : "";
    // console.log("!! mintString", mintString)
    ...
    const outfile = `${this.dir}/tmp/tx_${UID}.draft`;
    
    const execString = `${this.cliPath} transaction build-raw \
    ${txInString} \
    ${txOutString} \
    ${certs} \
    ${withdrawals} \
    ...
    // console.log("!! transactionBuildRaw >", execString, "<");
    execSync(execString);
    ...
    return outfile // `${this.dir}/tmp/tx_${UID}.raw`;
  }
  ...
  transactionCalculateMinFee(options) {
    ...
    const execString = `${this.cliPath} transaction calculate-min-fee \
    ...
    // console.log("!! transactionCalculateMinFee", execString);
  ...
  transactionSign(options) {
    ...
    const execString = `${this.cliPath} transaction sign \
      --tx-body-file ${options.txBody} \
      --${this.network} \
      ${signingKeys} \
      --out-file ${this.dir}/tmp/tx_${UID}.signed`;
    // console.log("!! transactionSign", execString);
    execSync(execString);
  ...
  transactionSubmit(tx) {
    ...
    const execString = `${this.cliPath} transaction submit --tx-file ${parsedTx} --${this.network}`;
    console.log("!! transactionSubmit:", execString)
    execSync(execString);
